...
/* eslint-disable @next/next/no-img-element -- DepthCarousel positions each <img> from measured 3D card geometry, so next/image would break its layout math. */
import {
  useCallback,
  useEffect,
  useLayoutEffect,
...
const DepthCarousel = ({
  ...
  // Layout effect rather than a render-time write: every consumer reads these
  // from callbacks, effects or pointer handlers, all of which run after this.
  useLayoutEffect(() => {
  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      // Otherwise the carousel swallows vertical wheel events and the page
      // cannot be scrolled past it.
      if (wheelAxis === 'horizontal' && !horizontal) return;
      e.preventDefault();
      tweenRef.current?.kill();
    ...
    >
      <div className="absolute inset-0 [transform-style:preserve-3d]" ref={stageRef}>
        {data.map((item, i) => (
          <div
            key={i}
            className={`absolute left-1/2 top-1/2 cursor-pointer overflow-hidden ${cardSurface} shadow-[0_30px_60px_-20px_rgba(0,0,0,0.65),0_8px_20px_-10px_rgba(0,0,0,0.5)] [transform:translate(-50%,-50%)] [transform-origin:center] [will-change:transform,opacity,filter]`}
            ref={el => {
          ...
          >
            <img
              className={`block h-full w-full select-none [pointer-events:none] [-webkit-user-drag:none] ${imageFit === 'contain' ? 'object-contain' : 'object-cover'}`}
              src={item.image}
              alt={item.alt || ''}
              draggable={false}
            />
