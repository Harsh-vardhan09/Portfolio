...
const MaskedHeading: React.FC<MaskedHeadingProps> = ({
  ...
  useEffect(() => {
    sync();
  }, [sync, words, tag, align, weight, tracking, lineHeight, textScale, maxFontSize, nowrap]);
  ...
  useEffect(() => {
    ...
    const settle = () => {
      ...
      gsap.set(layer, {
        opacity: 1,
        scale: 1,
        clipPath: reveal === 'wipe' ? 'inset(0% 0% 0% 0%)' : 'none'
      });
    };
    ...
    >
      ...
      <span ref={revealRef} className="absolute inset-0 block pointer-events-none">
        <span className="absolute inset-0 block" style={{ clipPath: `url(#${clipId})` }}>
          {fillColor ? <span className="absolute inset-0 block" style={{ background: fillColor }} /> : null}
          <span
            ref={mediaRef}
